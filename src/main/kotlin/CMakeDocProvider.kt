...
class CMakeDocProvider : AbstractDocumentationProvider() {
  ...
  private val asciiDoc by lazy { Asciidoctor.Factory.create() }

  fun runCMake(vararg args: String): Process {
    val cmake = CPPToolchains.getInstance().defaultToolchain?.cMake!!
  ...
  override fun generateDoc(element: PsiElement?, originalElement: PsiElement?): String? {
    return if (element == null || element !is CMakeElement) {
      ...
      when (element) {
        ...
        if (it == null) null else asciiDoc.render(it, OptionsBuilder.options().inPlace(true))
      }
    }
  }
}
